...
- `cargo fmt --all --check` is the first Rust CI step. Unformatted new Rust fails both Linux and Windows jobs before Clippy or tests run; apply `cargo fmt --all` in the same change.
...
- Windows cannot `File::set_len(0)` on a handle opened with `.append(true)` (`PermissionDenied` / `SetEndOfFile` on `FILE_APPEND_DATA`). Close the handle, truncate, then reopen append so Diagnostics delete keeps logging. Dependency status tests must write `mihomo_file_name()` (`bin/mihomo.exe` on Windows). Do not hardcode Unix `/tmp/...` strings in Windows `reveal_command` assertions; use `path.to_string_lossy()`.
