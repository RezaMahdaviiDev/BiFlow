...
export const desktop = {
  ...
  cancel(operationId: string): Promise<boolean> {
    return native ? invoke("cancel_operation", { operationId }) : mockApi.cancel(operationId);
  },
  getSettings(): Promise<AppConfig> {
    return native ? invoke("get_settings") : mockApi.getSettings();
  },
  validateSettings(draft: AppConfig): Promise<ValidationIssue[]> {
    return native ? invoke("validate_settings", { draft }) : mockApi.validateSettings(draft);
  },
  ...
  addRule(input: string, expectedRevision: number): Promise<DirectRulesDocument> {
    return native
      ? invoke("add_direct_rule", { input, expectedRevision })
      : mockApi.addRule(input, expectedRevision);
  },
  removeRule(input: string, expectedRevision: number): Promise<DirectRulesDocument> {
    return native
  ...
  testRoute(target: string): Promise<RouteTestResult> {
    return native ? invoke("test_route", { target }) : mockApi.testRoute(target);
  },
  diagnostics(): Promise<DiagnosticsReport> {
    return native ? invoke("run_full_diagnostics") : mockApi.diagnostics();
  },
  queryLogs(): Promise<LogEntry[]> {
    return native ? invoke("query_logs", { maximum: 500 }) : mockApi.queryLogs();
  },
  ...
  async subscribe(listener: (snapshot: StackSnapshot) => void): Promise<() => void> {
    if (!native) return mockApi.subscribe(listener);
    const unlisten = await listen<StackSnapshot>("stack-snapshot", (event) => listener(event.payload));
    return unlisten;
  },
};
