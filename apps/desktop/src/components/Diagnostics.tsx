import { CheckCircle2, CircleAlert, Download, FileText, LoaderCircle, Play } from "lucide-react";
import { useEffect, useState } from "react";
import { desktop } from "../api/desktop";
import type { DiagnosticsReport, ExportResult, LogEntry } from "../api/models";
...
export function Diagnostics({ report }: { report: DiagnosticsReport | null }) {
  ...
  useEffect(() => {
    void desktop.queryLogs().then(setLogs).catch(() => setLogs([]));
  }, [report]);
  ...
  return (
    <section aria-labelledby="diagnostics-title" className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <header>
          <h1 id="diagnostics-title" className="text-3xl font-semibold tracking-tight">
            Diagnostics
          </h1>
          <p className="mt-2 text-muted">
            Check the helper, upstream, providers, tunnel cleanup, and routing in one bounded run.
          </p>
        </header>
        <button
          type="button"
      ...
      <div className="rounded-2xl border border-ink/10 bg-surface p-4">
        ...
        {!report ? (
          <p className="text-sm text-muted">No diagnostic run in this session.</p>
        ) : (
          <ol className="space-y-3">
            {report.steps.map((step) => (
              <li key={step.id} className="flex items-start gap-3">
                {step.status === "passed" ? (
                  <CheckCircle2 className="mt-0.5 text-success" size={19} aria-hidden />
                ) : step.status === "warning" || step.status === "failed" ? (
                  <CircleAlert className="mt-0.5 text-amber-500" size={19} aria-hidden />
                ) : (
                  <LoaderCircle className="mt-0.5 text-muted" size={19} aria-hidden />
                )}
                <div>
        ...
        <div className="max-h-72 overflow-auto rounded-xl bg-canvas p-3 font-mono text-xs">
          {visibleLogs.length === 0 ? (
            ...
            visibleLogs.map((entry, index) => (
              <p key={`${entry.timestamp}-${index}`} className="mb-1 whitespace-pre-wrap break-all">
                <span className="text-muted">{entry.timestamp}</span> {entry.level.toUpperCase()} {entry.event}{" "}
      ...
      <div className="rounded-2xl border border-ink/10 bg-surface p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">
              Versions, redacted config, state, bounded logs, and diagnostic results only.
            </p>
          </div>
          <button
            type="button"
        ...
        {exported ? (
          <div className="mt-4 rounded-xl bg-canvas p-3 text-sm" role="status">
            ...
            <p className="mt-1 text-muted">Included: {exported.files.join(", ")}</p>
          </div>
        ) : null}
      </div>
    </section>
